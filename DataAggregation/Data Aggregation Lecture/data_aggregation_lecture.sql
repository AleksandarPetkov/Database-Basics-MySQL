...
/*
SELECT department_id, COUNT(first_name) AS 'Number from employees' 
FROM employees
GROUP BY  department_id;

SELECT DISTINCT department_id, round(AVG(salary), 2) AS `Avarage salary`
FROM employees
GROUP BY e.department_id;
*/
SELECT DISTINCT department_id, round(min(salary), 2) AS `Min salary`
FROM employees AS e
GROUP BY e.department_id
